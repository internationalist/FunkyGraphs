using System;
using UnityEngine;

public class Axis2D : MonoBehaviour
{
	public float axisLength;
	public enum AxisType {vertical, horizontal};
	public float lineBreadth = .05f;
	public AxisType axisType;


	public void Start()
	{
		MeshRenderer meshRenderer = gameObject.AddComponent<MeshRenderer>();
		meshRenderer.sharedMaterial = new Material(Shader.Find("UI/Default"));

		MeshFilter meshFilter = gameObject.AddComponent<MeshFilter>();

		Mesh mesh = new Mesh();
		Vector3[] vertices = null;
		if (axisType == AxisType.horizontal) {
			vertices = HorizontalVertices ();
		} else {
			vertices = VerticalVertices ();			
		}

		mesh.vertices = vertices;

		int[] tris = new int[6]
		{
			// lower left triangle
			0, 2, 1,
			// upper right triangle
			2, 3, 1
		};
		mesh.triangles = tris;

		Vector3[] normals = new Vector3[4]
		{
			-Vector3.forward,
			-Vector3.forward,
			-Vector3.forward,
			-Vector3.forward
		};
		mesh.normals = normals;

		Vector2[] uv = new Vector2[4]
		{
			new Vector2(0, 0),
			new Vector2(1, 0),
			new Vector2(0, 1),
			new Vector2(1, 1)
		};
		mesh.uv = uv;

		meshFilter.mesh = mesh;
		meshRenderer.sharedMaterial.color = Color.red;


	}

	Vector3[] HorizontalVertices ()
	{
		Vector3[] vertices = new Vector3[4] {
			new Vector3 (0, 0, 0),
			new Vector3 (axisLength, 0, 0),
			new Vector3 (0, lineBreadth, 0),
			new Vector3 (axisLength, lineBreadth, 0)
		};
		return vertices;
	}

	Vector3[] VerticalVertices ()
	{
		Vector3[] vertices = new Vector3[4] {
			new Vector3 (0, 0, 0),
			new Vector3 (lineBreadth, 0, 0),
			new Vector3 (0, axisLength, 0),
			new Vector3 (lineBreadth, axisLength, 0)
		};
		return vertices;
	}

}